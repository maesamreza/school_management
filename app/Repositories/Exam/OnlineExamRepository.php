...
class OnlineExamRepository
{
    ...
    public function addQuestion(OnlineExam $online_exam, $params = array())
    {
        ...
        $online_exam_question = $this->online_exam_question->forceCreate($this->formatQuestionParams($online_exam, $params));
    ...
    public function updateQuestion(OnlineExamQuestion $online_exam_question, $params = array())
    {
        ...
        $online_exam_question->forceFill($this->formatQuestionParams($online_exam_question->onlineExam, $params, $online_exam_question->id))->save();
     ...
     * @return array
     */
    private function formatQuestionParams($online_exam, $params = array(), $online_exam_question_id = null)
    {
    ...
    {
        if ($online_exam->status === 'expired') {
            throw ValidationException::withMessages(['message' => trans('exam.online_exam_expired')]);
        }

        if ($online_exam->status != 'running') {
            throw ValidationException::withMessages(['message' => trans('exam.online_exam_not_running')]);
        }
